#include "./includes/FireMonster.h"
#include <raylib.h>
#include <iostream>
using namespace std;

FireMonster::FireMonster(CharacterImage img)
{
    Init(1300.f, 200.f, 406, 339, 200.f, 1000.f, KEY_F, true, img);
    FFires.resize(MAX_BULLETS);
}

void FireMonster::Shoot()
{
    Texture2D fireTexture = LoadTexture("img/level2/IceFire.png");
    Vector2 spawn = {
        x + (direction == 1 ? image.gunOffset.x : width - image.gunOffset.x),
        y + image.gunOffset.y};
    FFires[nextBullet] = Fire(spawn, direction, 15, fireTexture);
    nextBullet = (nextBullet + 1) % MAX_BULLETS;
}

void FireMonster::Update(float dt, const vector<Platform> &plats, float winW, float worldW)
{
    ...
    // 7) Flame Fires update
    for (auto &b : FFires)
        b.Update(dt);
}

void FireMonster::Draw(const Camera2D &cam) const
{
    Texture2D tex;
    if (jumping)
...
void FireMonster::Unload() { image.Unload(); }